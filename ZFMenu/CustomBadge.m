...
- (id) initWithString:(NSString *)badgeString withScale:(CGFloat)scale withShining:(BOOL)shining
{
	...
	if(self!=nil) {
		...
		self.badgeFrame = YES;
		self.badgeFrameColor = kUIColorFromRGB(0xff2300);
		self.badgeInsetColor = kUIColorFromRGB(0xff2300);
		self.badgeCornerRoundness = 0.4;
		self.badgeScaleFactor = scale;
		self.badgeShining = shining;
		[self autoBadgeSizeWithString:badgeString withFont:[UIFont systemFontOfSize:12.0]];
	}
	return self;
}

- (id) initWithString:(NSString *)badgeString withScale:(CGFloat)scale withShining:(BOOL)shining withFont:(UIFont *)font
{
    self = [super initWithFrame:CGRectMake(0, 0, 25, 25)];
    if(self!=nil) {
        self.contentScaleFactor = [[UIScreen mainScreen] scale];
        ...
        self.badgeScaleFactor = scale;
        self.badgeShining = shining;
        [self autoBadgeSizeWithString:badgeString withFont:font];
    }
    return self;
}
...
- (void) autoBadgeSizeWithString:(NSString *)badgeString withFont:(UIFont *)font
{
    CGSize retValue;
    CGFloat rectWidth, rectHeight;
    CGSize stringSize = [badgeString sizeWithAttributes:@{NSFontAttributeName:font}];
    CGFloat flexSpace;
    if ([badgeString length]>=3) {
        flexSpace = [badgeString length];
        rectWidth = 12 + (stringSize.width + flexSpace); rectHeight = 25;
...
//given text and font
+ (CustomBadge*) customBadgeWithString:(NSString *)badgeString withFont:(UIFont *)font
{
    return [[self alloc] initWithString:badgeString withScale:1.0 withShining:NO withFont:font];
}
